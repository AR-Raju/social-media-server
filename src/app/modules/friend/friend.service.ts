...
const sendFriendRequestIntoDB = async (senderId: string, targetUserId: string, message?: string) => {
  ...
  if (senderId === targetUserId) {
    throw new AppError(httpStatus.BAD_REQUEST, "Cannot send friend request to yourself")
  }

  // Check if already friends
  const sender = await User.findById(senderId)
  if (sender?.friends.includes(targetUserId)) {
    throw new AppError(httpStatus.BAD_REQUEST, "You are already friends with this user")
  }
...
const acceptFriendRequestIntoDB = async (userId: string, requestId: string) => {
  ...
  if (friendRequest.recipient.toString() !== userId) {
    throw new AppError(httpStatus.FORBIDDEN, "You can only accept requests sent to you")
  }
...
const rejectFriendRequestIntoDB = async (userId: string, requestId: string) => {
  ...
  // Check if user is the recipient
  if (friendRequest.recipient.toString() !== userId) {
    throw new AppError(httpStatus.FORBIDDEN, "You can only reject requests sent to you")
  }
...
const removeFriendFromDB = async (userId: string, friendUserId: string) => {
  ...
  if (!user.friends.includes(friendUserId)) {
    throw new AppError(httpStatus.BAD_REQUEST, "You are not friends with this user")
  }
...
const getFriendsListFromDB = async (userId: string, query: Record<string, unknown>) => {
  const user = await User.findById(userId)
  if (!user) {
    throw new AppError(httpStatus.NOT_FOUND, "User not found")
  }
  ...
  const friendQuery = new QueryBuilder(
    User.find({ _id: { $in: user.friends } }).select("name avatar bio location isActive lastSeen"),
    query,
  )
    .search(["name", "bio"])
    .filter()
...
const getFriendRequestsFromDB = async (userId: string, query: Record<string, unknown>) => {
...
const getSentFriendRequestsFromDB = async (userId: string, query: Record<string, unknown>) => {
  const requestQuery = new QueryBuilder(
...
const getFriendSuggestionsFromDB = async (userId: string, limit = 10) => {
  ...
  const excludeIds = [userId, ...user.friends, ...user.sentFriendRequests, ...user.friendRequests]
