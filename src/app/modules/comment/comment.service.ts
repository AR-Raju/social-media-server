...
const createCommentIntoDB = async (userId: string, postId: string, commentData: TComment) => {
  ...
  if (!canComment) {
    throw new AppError(httpStatus.FORBIDDEN, "You don't have permission to comment on this post")
  }
  ...
  if (commentData.parentComment) {
    ...
    if (parentComment.post.toString() !== postId) {
      throw new AppError(httpStatus.BAD_REQUEST, "Parent comment does not belong to this post")
    }
  }
  ...
  if (commentData.parentComment) {
    ...
    if (parentComment && parentComment.author.toString() !== userId) {
      await Notification.create({
        ...
        data: { postId, commentId: comment._id, parentCommentId: commentData.parentComment },
      })
    }
  }
...
const getPostCommentsFromDB = async (userId: string, postId: string, query: Record<string, unknown>) => {
  ...
  if (!canView) {
    throw new AppError(httpStatus.FORBIDDEN, "You don't have permission to view this post's comments")
  }
...
const getCommentRepliesFromDB = async (userId: string, commentId: string, query: Record<string, unknown>) => {
  ...
  if (!canView) {
    throw new AppError(httpStatus.FORBIDDEN, "You don't have permission to view this comment's replies")
  }

  const replyQuery = new QueryBuilder(
    Comment.find({ parentComment: commentId }).populate("author", "name avatar isActive").sort({ createdAt: 1 }),
    query,
  )
    .search(CommentSearchableFields)
    .filter()
...
const updateCommentIntoDB = async (userId: string, commentId: string, payload: Partial<TComment>) => {
...
const reactToCommentIntoDB = async (userId: string, commentId: string, reactionType: string) => {
  const comment = await Comment.findById(commentId).populate("post")
  if (!comment) {
    throw new AppError(httpStatus.NOT_FOUND, "Comment not found")
  }
  ...
  const canView = await canUserViewPost(userId, comment.post)
  if (!canView) {
    throw new AppError(httpStatus.FORBIDDEN, "You don't have permission to react to this comment")
  }
  ...
  if (existingReaction) {
    if (existingReaction.type === reactionType) {
      ...
      await CommentReaction.findByIdAndUpdate(existingReaction._id, { type: reactionType })
      await Comment.findByIdAndUpdate(commentId, {
        $pull: { [`reactions.${existingReaction.type}`]: userId },
        $push: { [`reactions.${reactionType}`]: userId },
      })
      return { action: "updated", type: reactionType, oldType: existingReaction.type }
    }
  } else {
    // Add new reaction
    ...
    if (comment.author.toString() !== userId) {
      await Notification.create({
        ...
        data: { postId: comment.post._id, commentId },
      })
    }
...
const getSingleCommentFromDB = async (userId: string, commentId: string) => {
  ...
  const canView = await canUserViewPost(userId, comment.post)
  if (!canView) {
    throw new AppError(httpStatus.FORBIDDEN, "You don't have permission to view this comment")
  }
...
// Helper functions
const canUserCommentOnPost = async (userId: string, post: any): Promise<boolean> => {
