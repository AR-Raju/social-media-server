...
const getAllGroupsFromDB = async (query: Record<string, unknown>) => {
  const groupQuery = new QueryBuilder(
    Group.find({ isActive: true }).populate("admin", "name avatar").select("-members -memberRequests -bannedMembers"),
    query,
  )
...
const getSingleGroupFromDB = async (userId: string, groupId: string) => {
  ...
  if (group.privacy === "secret" && !group.members.includes(userId)) {
    throw new AppError(httpStatus.FORBIDDEN, "You don't have permission to view this group")
  }

  return group
}

const updateGroupIntoDB = async (userId: string, groupId: string, payload: Partial<TGroup>) => {
  const group = await Group.findById(groupId)
  if (!group) {
    throw new AppError(httpStatus.NOT_FOUND, "Group not found")
  }
  ...
  if (group.admin.toString() !== userId && !group.moderators.includes(userId)) {
    throw new AppError(httpStatus.FORBIDDEN, "You don't have permission to update this group")
  }

  const updatedGroup = await Group.findByIdAndUpdate(groupId, payload, { new: true, runValidators: true })
    .populate("admin", "name avatar")
    .populate("moderators", "name avatar")
...
const deleteGroupFromDB = async (userId: string, groupId: string) => {
  ...
  if (group.admin.toString() !== userId) {
    throw new AppError(httpStatus.FORBIDDEN, "Only group admin can delete the group")
  }
...
const getGroupPostsFromDB = async (userId: string, groupId: string, query: Record<string, unknown>) => {
  const group = await Group.findById(groupId)
  if (!group) {
    throw new AppError(httpStatus.NOT_FOUND, "Group not found")
  }
  ...
  if (group.privacy === "secret" && !group.members.includes(userId)) {
    throw new AppError(httpStatus.FORBIDDEN, "You don't have permission to view this group's posts")
  }

  if (group.privacy === "private" && !group.members.includes(userId)) {
    throw new AppError(httpStatus.FORBIDDEN, "You must be a member to view this group's posts")
  }
...
const getUserGroupsFromDB = async (userId: string, query: Record<string, unknown>) => {
  const user = await User.findById(userId)
  if (!user) {
    throw new AppError(httpStatus.NOT_FOUND, "User not found")
  }
  ...
  const groupQuery = new QueryBuilder(
    Group.find({ _id: { $in: user.groups } })
      .populate("admin", "name avatar")
      .select("-members -memberRequests -bannedMembers"),
    query,
  )
    .search(GroupSearchableFields)
    .filter()
...
const approveJoinRequestIntoDB = async (userId: string, groupId: string, requestUserId: string) => {
  const group = await Group.findById(groupId)
  if (!group) {
    throw new AppError(httpStatus.NOT_FOUND, "Group not found")
  }
  ...
  // Check if user is admin or moderator
  if (group.admin.toString() !== userId && !group.moderators.includes(userId)) {
    throw new AppError(httpStatus.FORBIDDEN, "You don't have permission to approve join requests")
  }
