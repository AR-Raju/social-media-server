...
const addReactionIntoDB = async (
  ...
  if (!canReact) {
    throw new AppError(httpStatus.FORBIDDEN, `You don't have permission to react to this ${targetType}`)
  }
  ...
  if (existingReaction) {
    if (existingReaction.type === reactionType) {
      ...
      await Reaction.findByIdAndUpdate(existingReaction._id, { type: reactionType })
      ...
      result = { action: "updated", type: reactionType, oldType: existingReaction.type }
    }
  } else {
    // Add new reaction
  ...
  if (targetType === "post") {
    SocketService.sendPostReaction(targetId, userId, reactionType, result.action)
  }
...
const getReactionsFromDB = async (
  ...
  if (!canView) {
    throw new AppError(httpStatus.FORBIDDEN, `You don't have permission to view reactions on this ${targetType}`)
  }

  const reactionQuery = new QueryBuilder(
    Reaction.find({ target: targetId, targetType }).populate("user", "name avatar").sort({ createdAt: -1 }),
    query,
  )
    .filter()
    .sort()
...
const getUserReactionsFromDB = async (userId: string, query: Record<string, unknown>) => {
  const reactionQuery = new QueryBuilder(
    Reaction.find({ user: userId }).populate("target").populate("user", "name avatar").sort({ createdAt: -1 }),
    query,
  )
    .filter()
    .sort()
...
const removeReactionFromDB = async (userId: string, targetId: string, targetType: "post" | "comment") => {
...
// Helper functions
const canUserReactToTarget = async (userId: string, target: any, targetType: "post" | "comment"): Promise<boolean> => {
  if (targetType === "post") {
...
const canUserViewTarget = async (userId: string, target: any, targetType: "post" | "comment"): Promise<boolean> => {
  if (targetType === "post") {
    return canUserViewPost(userId, target)
  } else {
    return canUserViewPost(userId, target.post)
